    ...
    <>
      ...
      <section className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {selectedListings.map((property) => (
          <article key={property.title} className="rounded-[2rem] border border-[#e7dcc8] bg-white p-6 shadow-[0_18px_50px_rgba(9,21,64,0.08)]">
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-[#071426] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-white">{property.tag}</span>
              <span className="rounded-full border border-[#e7dcc8] bg-[#fbf8f0] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#8a6428]">Best for {property.bestFor}</span>
            </div>
            ...
            <div className="mt-4 grid gap-3 text-sm">
              <div className="rounded-2xl border border-[#e7dcc8] bg-[#fbf8f0] p-4">
                ...
                <div className="mt-2 font-semibold text-[#071426]">{property.price}</div>
              </div>
              <div className="rounded-2xl border border-[#e7dcc8] bg-white p-4">
                ...
                <div className="mt-2 font-semibold text-[#071426]">{property.size}</div>
              </div>
            </div>
          </article>
        ))}
            ...
            <div className="grid" style={{ gridTemplateColumns: `190px repeat(${selectedListings.length}, minmax(210px, 1fr))` }}>
              ...
              {selectedListings.map((property) => (
                <div key={property.title} className="border-r border-[#e5dcc9] p-5 last:border-r-0">
                  ...
                  <div className="mt-2 inline-flex rounded-full border border-[#e7dcc8] bg-[#fbf8f0] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8a6428]">{property.type}</div>
                </div>
              ))}
            </div>
