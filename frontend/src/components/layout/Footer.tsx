...
export default function Footer() {
  return (
    <footer className="mt-10 w-full overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(215,190,138,0.12),_transparent_20%),linear-gradient(160deg,_#071426_0%,_#0D2342_58%,_#13273f_100%)] text-white">
      <div className="mx-auto max-w-[1500px] px-6 py-12 md:px-10 lg:px-14 lg:py-14">
        <div className="grid gap-10 lg:grid-cols-[1.5fr_0.85fr_0.85fr_0.95fr] lg:gap-10">
          <div>
            <Image src={brand.logo} alt={brand.name} width={260} height={112} className="h-16 w-auto object-contain" />
            <p className="mt-4 max-w-md text-sm leading-7 text-white/68">{brand.name} presents model houses and housing options with clarity, trust, and editorial restraint.</p>
            <form className="mt-6 max-w-md rounded-[1.35rem] border border-white/10 bg-white/[0.06] p-2 backdrop-blur-sm">
              <label className="sr-only" htmlFor="footer-email">Your email for updates</label>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <input id="footer-email" name="email" type="email" autoComplete="email" className="min-h-11 w-full rounded-full border border-white/10 bg-transparent px-4 text-sm text-white placeholder:text-white/45 outline-none transition focus:border-[#d7be8a]/60" placeholder="Your email for updates" />
                <button type="submit" className="min-h-11 shrink-0 rounded-full bg-[#d7be8a] px-5 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#071426] transition hover:bg-[#e4cf9f]">Subscribe</button>
              </div>
            </form>
          </div>
